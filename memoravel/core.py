...
class Memoravel:
    def __init__(self, limit=10, max_tokens=8000, preserve_initial_memories=0, preserve_system_memories=False):
    ...
    def _trim_history(self):
        ...
        ) and self._has_removable_memory(removable_start_index):
    ...
    def _has_removable_memory(self, start_index):
        """Verifica se há mensagens removíveis a partir de um índice dado."""
        return any(
            (msg["role"] != "system" or not self.preserve_system_memories)
            for msg in self.history[start_index:]
        )
